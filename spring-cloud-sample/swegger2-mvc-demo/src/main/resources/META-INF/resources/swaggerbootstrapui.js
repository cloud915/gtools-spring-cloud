...
(function ($) {
    ...
    SwaggerBootstrapUi.prototype.createDetailMenu=function () {
        ...
        //响应状态码定义
        var responseCodesLi=$("<li id='responseCodesLi' class=\"detailMenu\"><a href=\"javascript:void(0)\"><i class=\"icon-text-width iconfont icon-zhongduancanshuguanli\"></i><span class=\"menu-text\"> 响应状态码定义 </span></a></li>");
        responseCodesLi.on("click",function () {
            that.getMenu().find("li").removeClass("active");
            responseCodesLi.addClass("active");
            that.createResponseCodesElement();
        });
        that.getMenu().append(responseCodesLi);
        //离线文档功能
        ...
        $.each(that.currentInstance.tags,function (i, tag) {
            ...
            if(len==0){
                ...
                var ul=$('<ul class="submenu"></ul>')
                $.each(tag.childrens,function (i, children) {
                    var childrenLi=$('<li class="menuLi" ><div class="mhed">' +
                        '<div class="swu-hei">' +
                        '<span class="swu-menu swu-left">' +
                            '<span class="menu-url-'+children.methodType.toLowerCase()+'">'+children.methodType.toUpperCase()+'</span>' +
                        '</span>' +
                        '<span class="swu-menu swu-left">' +
                        '   <span class="menu-url">'+children.summary+'</span>' +
                        '</span>' +
                        '</div></li>');
                    childrenLi.data("data",children);
                    ul.append(childrenLi);
     ...
     * 创建响应状态码tab
     */
    SwaggerBootstrapUi.prototype.createResponseCodesElement=function () {
        var that=this;
        var layui=that.layui;
        var element=layui.element;
        var tabId = "responseCodes-tab";
        ...
        $.ajax({
            url:"/apiDoc/respCodes",
            method:"get",
            dateType:"json",
            success:function(responseCodes){
                var html = template('responseCodesScript',{responseCodes:responseCodes});
                var tabObj={
                    id:tabId,
                    title:'全局参数设置',
                    content:html
    ...
    SwaggerBootstrapUi.prototype.createApiInfoTable=function (apiInfo,menu) {
        ...
        if(that.tabExists(tabId)){
            ...
            if(apiInfo.parameters!=null&&apiInfo.parameters.length>0){
                for(var x in apiInfo.parameters){
                    var parameter = apiInfo.parameters[x];
                    var inType = parameter["in"];
                    if(inType=="body"){
                        pids[parameter.id]=true;
                        isBody = true;
                    }else {
                        if(inType=="query" || inType=="form"){
                            isForm = true;
                        }
                        data.push(parameter);
                    }
            ...
            if(isForm){
                apiInfo.consumes = ["application/x-www-form-urlencoded"];
            }
            ...
            if(apiInfo.refparameters!=null&&apiInfo.refparameters.length>0){
                $.each(apiInfo.refparameters,function (i, ref) {
                    for(var x in ref.params){
                        var parameter = ref.params[x];
                        if(pids[parameter.pid]){
                            parameter.pid = -1;
                        }
                        data.push(parameter);
                    }
            ...
            var markdown =
                    "**接口地址** `"+ apiInfo.showUrl + "`\n\n"
                   +"**请求方式** `"+ apiInfo.methodType + "`\n\n"
                   +"**请求类型** `"+ (apiInfo.consumes||[]).join(",")  + "`\n\n"
                   +"**响应类型** `"+ (apiInfo.produces.toString()=="*/*"?"application/json;charset=utf-8":apiInfo.produces.join(",")) + "`\n\n"
                   +"**接口描述** `"+ (apiInfo.description||"-")  + "`\n\n"
                   +"**请求参数** \n\n";
            markdown += markdownTreeTable(requestTableId);
            markdown += "**响应参数** \n\n";
            markdown += markdownTreeTable(responseTableId);
            markdown += "**响应示例** \n\n";
            markdown += "```json\n";
            markdown += JSON.stringify(apiInfo.responseJson,null,"\t");
            markdown += "\n```\n";
            //初始化copy按钮功能
    ...
    function markdownTreeTable(treeTableId){
        var requestTable =$("#"+treeTableId+"+div .layui-table-header>table");
        var trHeader = requestTable.find("th");
        var markdown = "|";
        var separator  ="|---";

        trHeader.each(function(i,td){
            markdown = markdown + $(td).text()+ "|" ;
            separator += "---|"
        });
        markdown = markdown + "\n" + separator + "\n";
        requestTable =$("#"+treeTableId+"+div .layui-table-body>table");
        var trs = requestTable.find("tr");
        ...
        trs.each(function(){
            ...
            var td0 = $(tds[0]);
            var span = td0.find(">div>span");
            var tid = span.attr("lay-tid");
            markdown =  markdown + "|" ;
            var tpid=  span.attr("lay-tpid");
            tds.each(function(i,td){
                var content = $(td).text().replace(/^\s+/g,"").replace(/\s+$/g,"") ;
                if(i==0){
                    while (ids.length>0 && ids[ids.length-1]!=tpid){
                        ids.pop();
                    }
                    content =  "&nbsp;".repeat(ids.length*8) + content;
                    ids.push(tid);
                }
